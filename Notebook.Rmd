...
We can see that we get p-values below 0.05 for 18 variables, thus, we can reject in these cases the null hypothesis and assume that these are not independent to SalePrice. 

Also, we can assume that these 23 other predictors are independent and do not affect the output of the units sold. In fact, 14 of them have a p-value over 0.9 indicating that they are with a probability over 99% independent on the SalePrice. 
...
The methods to use are those appropriate to study a regression problem such as this one. Thus, we will be using *Linear Models*, *Logistic Regression*, *Neural Network Models*, *Random Forest* or *Lasso Regression*.
